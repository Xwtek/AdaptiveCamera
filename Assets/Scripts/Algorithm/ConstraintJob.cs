...
namespace AdaptiveCamera.Algorithm
{
    public struct ConstraintJob : IJob{
        ...
        [BurstDiscard]
        public static void NanValue(float result, ConstraintType type){
            if (math.isnan(result))
            {
                throw new Exception("The value breaks for " + type.ToString());
            }
        }
        ...
        public static float GetCost(ConstraintData constraint, float3 camera, float3 playerPosition){
            switch (constraint.type)
            {
                ...
                case ConstraintType.VIEWPOINT:
                    /*
                    var constForward = math.mul(constraint.rotation, math.forward()).xz;
                    var cameraForward = math.mul(camera.angle, math.forward()).xz;
                    var angle = math.acos(math.dot(constForward, cameraForward));
                    totals[idx] += math.abs(angle) * constraint.cost / constraint.scale; */
                    {
                        var cameraForward = math.normalizesafe(playerPosition.xz - camera.xz);
                        var objDir = math.normalizesafe(constraint.position.xz - camera.xz);
                        var angle = math.acos(math.dot(cameraForward, objDir));
                        return NormalizeAngleCost(angle, constraint);
                    }
                case ConstraintType.HEIGHT:
                    {
                        var height = math.abs(constraint.position.y - camera.y);
                        return NormalizeCost(height, constraint);
                    }
